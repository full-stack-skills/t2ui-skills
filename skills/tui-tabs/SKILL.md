...
name: tui-tabs
license: Apache-2.0
description: Generate pixel-precise ASCII TUI for Tabs u-tabs with strict output blocks TUI_RENDER COMPONENT_SPEC PENCIL_SPEC PENCIL_BATCH_DESIGN suitable for Pencil MCP drawing workflows.
---


## Purpose

- Produce an ASCII Text UI (TUI) representation of **Tabs**.
...
Canonical `props` for tabs:
- `tabs: Array<{ id: string, label: string, disabled?: boolean }>`
- `activeId: string` (or `modelValue`)
- `scrollable?: boolean` (if many tabs, truncate labels and show `…`)

Rendering rules:
- Active tab uses brackets: `[Profile]` while inactive uses spaces: ` Settings `.
- Disabled tab uses parentheses: `(Billing)`.
Interaction hints:
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-tabs_ex1", "name": "Tabs", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 96 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-tabs_ex2", "name": "Tabs", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 104 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-tabs_ex3", "name": "Tabs", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 112 }, "zIndex": 1 }
  ]
}
```
...
cmpText=I(screen,{type:"text",name:"Tabs/Error",content:"Error: Something went wrong…"})
U(cmpText,{x:40,y:60,width:310,height:20,textColor:"#6b7280",fontFamily:"Inter",fontSize:14,fontWeight:400})
```

### Example 4 — Many tabs with truncation
...
```json
{
  ...
  "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 96 },
  "zIndex": 1,
  "layout": { "paddingPx": 12, "gapPx": 8, "align": "left", "valign": "top" },
  "style": { "fillColor": "#ffffff", "textColor": "#111111", "strokeColor": "#e5e7eb", "strokeThickness": 1, "cornerRadius": 12, "opacity": 1 },
  "typography": { "fontFamily": "Inter", "fontSize": 13, "fontWeight": 400, "lineHeight": 18 },
  "overflow": { "mode": "truncate", "ellipsis": "…", "maxLines": 1 },
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-tabs_ex4", "name": "Tabs", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 96 }, "zIndex": 1 }
  ]
}
```
...
CALL 2:
bg=I(screen,{type:"rect",name:"Tabs/Background"})
U(bg,{x:24,y:24,width:342,height:56,fillColor:"#ffffff",strokeColor:"#e5e7eb",strokeThickness:1,cornerRadius:12})
tabs=I(screen,{type:"text",name:"Tabs/Text",content:"[Profile]  Settings  Security  Notific…  (Billing)"})
U(tabs,{x:40,y:42,width:310,height:20,textColor:"#111111",fontFamily:"Inter",fontSize:13,fontWeight:400})
```
...
| tui-tabs | Tabs | 0 | 0 | 0 | 0 | 0 | keyProps=... | state=... | hotkeys=... |
```
