...
name: tui-nvue
license: Apache-2.0
description: Generate pixel-precise ASCII TUI for Nvue (u-nvue) with strict output blocks (TUI_RENDER, COMPONENT_SPEC, PENCIL_SPEC, PENCIL_BATCH_DESIGN) suitable for Pencil MCP drawing workflows.
---


## Purpose

- Produce an ASCII Text UI (TUI) representation of **Nvue**.
...
- `full-stack-skills/tui-sources/components.json` (lookup by `skill=tui-nvue` or `slug=nvue`)
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-nvue_ex1", "name": "Nvue", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 96 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-nvue_ex2", "name": "Nvue", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 104 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-nvue_ex3", "name": "Nvue", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 112 }, "zIndex": 1 }
  ]
}
```
...
cmpText=I(screen,{type:"text",name:"Nvue/Error",content:"Error: Something went wrong…"})
...
| tui-nvue | Nvue | 0 | 0 | 0 | 0 | 0 | keyProps=... | state=... | hotkeys=... |
```
