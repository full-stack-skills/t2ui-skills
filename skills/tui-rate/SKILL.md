...
name: tui-rate
license: Apache-2.0
description: Generate pixel-precise ASCII TUI for Rate u-rate with strict output blocks TUI_RENDER COMPONENT_SPEC PENCIL_SPEC PENCIL_BATCH_DESIGN suitable for Pencil MCP drawing workflows.
---


## Purpose

- Produce an ASCII Text UI (TUI) representation of **Rate**.
...
Canonical `props` for a rating control:
- `count: number` (default 5)
- `value: number` (or `modelValue`)
- `allowHalf: boolean` (half steps)
- `min: number` (minimum selectable)
- `iconOn: string` (render as `*` by default)
- `iconOff: string` (render as `.` by default)

Rendering rules:
- Full: `*`, half: `+`, empty: `.`
- Always print numeric value suffix: `(3.5/5)` for unambiguous logs.

Interaction hints:
- `<left>/<right>` adjust
- `<enter>` confirm
- Disabled: remove interaction hints and render as display-only.
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-rate_ex1", "name": "Rate", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 96 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-rate_ex2", "name": "Rate", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 104 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-rate_ex3", "name": "Rate", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 112 }, "zIndex": 1 }
  ]
}
```
...
cmpText=I(screen,{type:"text",name:"Rate/Error",content:"Error: Something went wrong…"})
U(cmpText,{x:40,y:60,width:310,height:20,textColor:"#6b7280",fontFamily:"Inter",fontSize:14,fontWeight:400})
```

### Example 4 — 3.5 out of 5 (display-only)
...
```json
{
  ...
  "typography": { "fontFamily": "Inter", "fontSize": 14, "fontWeight": 400, "lineHeight": 20 },
  "overflow": { "mode": "truncate", "ellipsis": "…", "maxLines": 1 },
  "state": { "focused": false, "disabled": true, "loading": false, "error": null },
  "hotkeys": []
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-rate_ex4", "name": "Rate", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 96 }, "zIndex": 1 }
  ]
}
```
...
| tui-rate | Rate | 0 | 0 | 0 | 0 | 0 | keyProps=... | state=... | hotkeys=... |
```
