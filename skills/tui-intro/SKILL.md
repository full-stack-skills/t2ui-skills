...
name: tui-intro
license: Apache-2.0
description: Generate pixel-precise ASCII TUI for Intro (u-intro) with strict output blocks (TUI_RENDER, COMPONENT_SPEC, PENCIL_SPEC, PENCIL_BATCH_DESIGN) suitable for Pencil MCP drawing workflows.
---


## Purpose

- Produce an ASCII Text UI (TUI) representation of **Intro**.
...
- `full-stack-skills/tui-sources/components.json` (lookup by `skill=tui-intro` or `slug=intro`)
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-intro_ex1", "name": "Intro", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 96 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-intro_ex2", "name": "Intro", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 104 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-intro_ex3", "name": "Intro", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 112 }, "zIndex": 1 }
  ]
}
```
...
cmpText=I(screen,{type:"text",name:"Intro/Error",content:"Error: Something went wrong…"})
...
| tui-intro | Intro | 0 | 0 | 0 | 0 | 0 | keyProps=... | state=... | hotkeys=... |
```
