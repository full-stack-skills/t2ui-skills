...
name: tui-common
license: Apache-2.0
description: Generate pixel-precise ASCII TUI for Common (u-common) with strict output blocks (TUI_RENDER, COMPONENT_SPEC, PENCIL_SPEC, PENCIL_BATCH_DESIGN) suitable for Pencil MCP drawing workflows.
---


## Purpose

- Produce an ASCII Text UI (TUI) representation of **Common**.
...
- `full-stack-skills/tui-sources/components.json` (lookup by `skill=tui-common` or `slug=common`)
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-common_ex1", "name": "Common", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 96 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-common_ex2", "name": "Common", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 104 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-common_ex3", "name": "Common", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 112 }, "zIndex": 1 }
  ]
}
```
...
cmpText=I(screen,{type:"text",name:"Common/Error",content:"Error: Something went wrong…"})
...
| tui-common | Common | 0 | 0 | 0 | 0 | 0 | keyProps=... | state=... | hotkeys=... |
```
