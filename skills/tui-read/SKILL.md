...
name: tui-read
license: Apache-2.0
description: Generate pixel-precise ASCII TUI for Read (u-read) with strict output blocks (TUI_RENDER, COMPONENT_SPEC, PENCIL_SPEC, PENCIL_BATCH_DESIGN) suitable for Pencil MCP drawing workflows.
---


## Purpose

- Produce an ASCII Text UI (TUI) representation of **Read**.
...
- `full-stack-skills/tui-sources/components.json` (lookup by `skill=tui-read` or `slug=read`)
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-read_ex1", "name": "Read", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 96 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-read_ex2", "name": "Read", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 104 }, "zIndex": 1 }
  ]
}
```
...
```json
{
  ...
  "nodes": [],
  "components": [
    { "id": "tui-read_ex3", "name": "Read", "bbox": { "topPx": 24, "leftPx": 24, "widthPx": 342, "heightPx": 112 }, "zIndex": 1 }
  ]
}
```
...
cmpText=I(screen,{type:"text",name:"Read/Error",content:"Error: Something went wrong…"})
...
| tui-read | Read | 0 | 0 | 0 | 0 | 0 | keyProps=... | state=... | hotkeys=... |
```
